                                ...
                                )
{
    ...
    *compressed_size = 0;
    ...
    printf("ccc try int = %i max = %i compressed = %i\n",input_len,max_output_len,result);
		...
		uint64_t *constant_factor, double *linear_factor, double *capped_linear_factor, uint64_t *capped_linear_cap)
{
    ...
    printf("overhead %llu  size = %llu\n",max_overhead,input_size);
                                ...
                                int *wrote_to_shared_buffer)
{
    ...
    for( p = 0; p < num_param; ++p )
    {
        ...
        if( strcmp( param->key, "lvl") == 0 )
        {
            ...
            if( compress_level > 9 )
                compress_level = 9;
            printf("Blosc: invalid compression level %s, switch to lvl %i\n",param->value, compress_level);
        }
        ...
        else if( strcmp( param->key, "shuffle") == 0 )
        {
            ...
            {
                printf("Blosc: unknown shuffle %s, disable shuffle\n",param->value);
            }
        }
        else if( strcmp( param->key, "threads") == 0 )
        {
            int threads = atoi( param->value );
        ...
        else if( strcmp( param->key, "compressor") == 0 )
        {
            ...
            {
                printf("Blosc: unknown compressor %s, switch to memcpy\n",param->value);
                strcpy( compressor, "memcpy" );
            }
        }
        else
        {
            adios_error(err_invalid_argument, "An unknown Blosc compression mode '%s' was specified for variable %s. "
                        "Available choices are: lvl, min_block, suffle, threads, compressor.\n",
                        param->key, var->name);
            return 0;
        }
    }
